  ...
  return (
    ...
    >
        ...
        <div className="mt-2 grid grid-cols-2 gap-2 md:grid-cols-3 lg:grid-cols-6">
          {stir.path.imply && (
            <div className="col-span-2 rounded border border-border bg-background/40 px-2 py-1.5 md:col-span-3 lg:col-span-6">
              <div className="text-[9px] text-muted-foreground">STRIP IMPLIES</div>
          ...
          <div className="rounded border border-border px-2 py-1.5">
            <div className="text-[9px] text-muted-foreground">SOFR spot</div>
            <div className="text-sm font-bold">{stir.sofr != null ? `${stir.sofr.toFixed(2)}%` : '—'}</div>
          </div>
          <div className="rounded border border-border px-2 py-1.5">
            <div className="text-[9px] text-muted-foreground">Path Δ (live)</div>
            <div className={`text-sm font-bold ${
              (stir.path.path_change_bps ?? 0) < 0 ? 'text-emerald-400' : 'text-amber-400'
            }`}>
          ...
          <div className="rounded border border-border px-2 py-1.5">
            <div className="text-[9px] text-muted-foreground">≈ 25bp cuts priced</div>
            <div className="text-sm font-bold text-foreground">
              {stir.path.approx_25bp_cuts_priced != null
                ? stir.path.approx_25bp_cuts_priced.toFixed(2)
                : '—'}
            </div>
          </div>
          <div className="rounded border border-border px-2 py-1.5">
            <div className="text-[9px] text-muted-foreground">Cut / hike path</div>
            <div className={`text-sm font-bold ${
              (stir.path.path_change_bps ?? 0) < 0 ? 'text-emerald-400' : 'text-amber-400'
            }`}>
              {stir.path.path_change_bps != null
            ...
            <div className="text-[8px] text-muted-foreground">path Δ in rate pts</div>
          </div>
          <div className="rounded border border-border px-2 py-1.5">
            <div className="text-[9px] text-muted-foreground">Front vs SOFR</div>
            <div className="text-sm font-bold">
          ...
          <div className="rounded border border-border px-2 py-1.5">
            <div className="text-[9px] text-muted-foreground">Implied end rate</div>
            <div className="text-sm font-bold">
      ...
      {stirChart.length > 0 ? (
        ...
        <div className="mt-3 rounded border border-border bg-background/40 p-3 text-[10px] text-muted-foreground">
      ...
      {liveSr3.length > 0 && (
        <div className="mt-3 space-y-3">
          <div className="rounded border border-border">
            <div className="flex flex-wrap items-center gap-2 border-b border-border bg-background/50 px-2 py-1 text-[10px] font-semibold text-foreground">
              <span>
            ...
            <div className="flex border-b border-border bg-card px-1.5 py-1 font-mono text-[9px] text-muted-foreground sticky top-0 z-[1]">
              ...
              renderRow={({ index, style, item: c }) => {
                ...
                return (
                  <div
                    ...
                    className={`flex items-center border-b border-border/40 px-1.5 font-mono text-[10px] hover:bg-muted/20 cursor-default ${
                      stirFocused && index === stirRow ? 'ring-1 ring-inset ring-primary/70 bg-primary/10' : ''
                    }`}
                  >
                    ...
                    <span className={`w-14 shrink-0 text-right tabular-nums ${
                      net == null ? '' : net >= 0 ? 'text-emerald-400' : 'text-red-400'
                    }`}>
                    ...
                    <span className={`w-14 shrink-0 text-right tabular-nums ${
                      vs != null && vs < 0 ? 'text-emerald-400' : vs != null && vs > 0 ? 'text-amber-400' : ''
                    }`}>
          ...
          <div className="grid grid-cols-1 gap-3 lg:grid-cols-2">
            <div className="overflow-x-auto rounded border border-border">
              <div className="border-b border-border bg-background/50 px-2 py-1 text-[10px] font-semibold text-foreground">
                  ...
                  <tbody>
                    {(stir?.sr1 || []).filter((c) => c.implied_rate != null).map((c) => (
                      <tr key={c.contract} className="border-t border-border/50">
                        ...
                        <td className={`p-1.5 text-right font-mono tabular-nums ${
                          c.net == null ? '' : c.net >= 0 ? 'text-emerald-400' : 'text-red-400'
                        }`}>
            ...
            <div className="overflow-x-auto rounded border border-border">
              ...
              {(stir?.treasury_futures || []).filter((c) => c.last_price != null).length === 0 ? (
                <div className="p-3 text-[10px] text-muted-foreground">No live Treasury futures</div>
              ) : (
                <table className="w-full border-collapse text-[10px]">
                  <thead>
                    <tr className="text-muted-foreground">
              ...
              {(stir?.sr3 || []).filter((c) => c.implied_rate != null).map((c) => {
                ...
                return (
                  <tr key={`path-${c.contract}`} className="border-t border-border/50">
                    ...
                    <td className={`p-1 text-right ${c.source === 'live' ? 'text-emerald-400' : 'text-amber-400'}`}>
                      {c.source}
                    </td>
                  </tr>
                );
